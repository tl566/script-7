...
//新，收取营业额
function smtg_receiveCoinNew(shopId, type = 7) {
  return new Promise((resolve) => {
    const body = {"type": type,shopId,"channel":1}
    const op = {
    ...
    $.get(op, async (err, resp, data) => {
      try {
        if (err) {
          ...
          if  ($.data.data.bizCode === 0) {
            if (type === 7) {
              console.log(`营业额收取成功，获得超市币：${data.data.result.receivedTurnover}，当前总超市币：${data.data.result.currentTurnover}`)
            } else {
              console.log(`营业额收取成功，获得金币：${data.data.result.receivedTurnover}，当前总获得金币：${data.data.result.currentTurnover}`)
            }
          } else {
            console.log(`${$.toStr(data)}\n`)
          }
      ...
      $.get(op, async (err, resp, data) => {
        try {
          if (err) {
            ...
            await receiveBlueCoinNew(3000, shopId, type);
          }
        } catch (e) {
          $.logErr(e, resp);
        } finally {
