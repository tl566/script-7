...
!(async () => {
  ...
  console.log('京喜领88元红包\n' +
      ...
      '助力逻辑：优先内部互助，若有剩余次数助力池互助\n' +
      '温馨提示：如提示助力火爆，可尝试寻找京东客服')
    let res = await getAuthorShareCode('http://adguard.ipq.co/jxhb.json')
    if (!res) {
       $.http.get({url: 'http://adguard.ipq.co/jxhb.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
       await $.wait(1000)
       res = await getAuthorShareCode('http://adguard.ipq.co/jxhb.json')
     }
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({
      url: `http://www.helpu.cf/jdcodes/getcode.php?type=jxlhb&num=0`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
