...
async function main() {
  try {
    ...
    if (Number($.waterWheelInfo['waterStorage']) > 0) {
      console.log(`收集风车水滴，现有${$.waterWheelInfo['waterStorage']}滴`);
      await takeGetRequest('collectWater');
    } else {
      console.log(`水车当前水滴为：${$.waterWheelInfo['waterStorage']},不可收取`);
    }
  } catch (e) {
    $.logErr(e)
  }
...
function dealReturn(type, data) {
  try {
    ...
    switch (type) {
      ...
      case 'collectWater':
        if (data.code === '0') {
          console.log(`收取水车水滴成功，当前已累计收集水车水滴${data.result.totalCollectWater}滴\n`);
        } else {
          console.log('收取水车水滴失败', JSON.stringify(data))
        }
        break;
      default:
