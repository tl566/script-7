...
function showMsg() {
  return new Promise(async resolve => {
    try {
      ...
      await $.wait(5000);
      await $.http.get({url: `https://purge.jsdelivr.net/gh/Dx3242/updateTeam@master/jd_updateTeam.json`}).then((resp) => {
...
function smtg_createPkTeam() {
  return new Promise((resolve) => {
    $.get(taskUrl('smtg_createPkTeam', { "teamName": `ZFeng00${$.index}`,  "channel": "1" }), (err, resp, data) => {
      try {
        if (err) {
          console.log('\n京小超: API查询请求失败 ‼️‼️')
