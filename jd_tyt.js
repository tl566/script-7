...
!(async () => {
  ...
  let res = await getAuthorShareCode('http://adguard.ipq.co/tyt.json')
  if (!res) {
    $.http.get({url: 'http://adguard.ipq.co/tyt.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('http://adguard.ipq.co/tyt.json')
  }
