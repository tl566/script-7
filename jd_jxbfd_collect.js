...
!(async () => {
  ...
  while (true) {
    count++
    console.log(`==========================第 ${count} 次挂机开始=======================`)
    console.time(`共${cookiesArr.length}个账号第 ${count} 次收集贝壳挂机耗时`)
    ...
    console.timeEnd(`共${cookiesArr.length}个账号第 ${count} 次收集贝壳挂机耗时`)
    console.log(`第 ${count} 次挂机结束，等待${Math.floor(60 / cookiesArr.length) || 2}秒，开始第 ${count + 1} 次挂机\n`);
    await $.wait((Math.floor(60 / cookiesArr.length) || 2) * 1000);
  }
  ...
  if (queryShell) {
    if (queryShell['iRet'] === 0) {
      ...
      if (NormShell && NormShell.length === 0) {
        console.log(`账号 ${$.index} ${$.UserName} 当前 海滩 暂无贝壳\n`);
        return
      }
      ...
      try {
        if (err) {
          ...
          if (data) {
            if (data['iRet'] === 0) {
              if (body) {
                console.log(`账号 ${$.index} ${$.UserName} 沙滩成功捡到一个 ${strType}，${data.Data.strFirstDesc}`);
              }
            } else {
              console.log(`账号 ${$.index} ${$.UserName} 沙滩捡${strType} 失败: ${data['sErrMsg']}, iRet: ${data['iRet']}`)
              if (data['iRet'] === 5403) {
                //东西过多，背包已放不下
                await sell(1, type);
                await $.wait(1000);
