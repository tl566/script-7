/*
新版京喜财富岛 挂机收集海滩贝壳
默认每180秒收集一次贝壳，可使用环境变量 SHELL_WAIT_TIME控制，最小值也必须为：每次海滩出现的总贝壳数量(10) * 5 的数值，即50秒的延迟时间。
更新日期：2021-07-21
 */
...
const shellWaitTime = $.isNode() ? (process.env.SHELL_WAIT_TIME ? process.env.SHELL_WAIT_TIME : 180) : 180;
if ($.isNode()) {
...
!(async () => {
  ...
  if (cookiesArr && cookiesArr.length && $.isNode()) {
    console.log(`\n沙滩收集贝壳 挂机开始，每过${shellWaitTime}秒自动收集一次\n`);
    //兼容iOS
    $.beginTimes = 0;
    setInterval(async () => {
...
//沙滩上捡贝壳
async function pickShells(ck, index) {
