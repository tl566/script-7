...
!(async () => {
  ...
  try {
    await getUA();
    for (let i = 0; i < cookiesArr.length; i++) {//cookiesArr.length
      $.hotFlag = false;
      $.index = i + 1;
      $.cookie = cookiesArr[i];
      $.isLogin = true;
...
async function takePostRequest(type) {
  ...
  switch (type) {
    case 'index':
      ...
      break;
    case 'getShareId':
      body = `appid=anniversary-celebra&functionId=jd_interaction_prod&body={"type":"${$.type}","apiMapping":"/api/supportTask/getShareId"}&t=${Date.now()}&loginType=2`;
      break;
    case 'doTask':
    ...
    case 'getReward':
      ...
      break;
    case 'doSupport':
      body = `appid=anniversary-celebra&functionId=jd_interaction_prod&body={"shareId":"${$.authorShareId}","apiMapping":"/api/supportTask/doSupport"}&t=${$.timeStamp}&loginType=2`;
      break;
    default:
      console.log(`错误${type}`);
  }
...
function dealReturn(type, data) {
  ...
  switch (type) {
    ...
    case 'getShareId':
      if (data && data.code === 200) {
        $.type === 99 ? console.log(`助力码1：${data.data || ''}`) : console.log(`助力码2：${data.data || ''}`);
        if ($.type === 100) shareList.push({'code': data.data, 'max': false, 'user': $.UserName});
        //if($.index < 10) shareList.push(data.data);
      } else {
