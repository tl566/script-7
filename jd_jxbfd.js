...
async function rewardSign() {
  ...
  if ($.TakeAggrPageData && $.TakeAggrPageData.hasOwnProperty('Sign')) {
    ...
    if (dwTodayStatus === 1) {
      ...
      console.log(`开始做 APP端【连续营业赢红包】任务，可获得京币：${ddwCoin}个，红包：${strDiscount}元`);
            ...
            if (data['iRet'] === 0) {
              ...
              if ($.TakeAggrPageData && $.TakeAggrPageData.hasOwnProperty('Sign') && type === 'xcx_user') {
                ...
                if (dwTodayStatus === 1) {
                  ...
                  console.log(`开始做 小程序端【连续营业赢红包】任务，可获得京币：${ddwCoin}个，红包：${strDiscount}元`);
                  const body = `ddwCoin=${ddwCoin}&ddwMoney=${ddwMoney}&dwPrizeType=${dwPrizeType}&strPrizePool=${strPrizePool}&dwPrizeLv=${dwPrizeLv}`;
