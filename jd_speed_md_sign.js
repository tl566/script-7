...
[task_local]
#京东极速版免单签到
20 0,22 * * * jd_speed_md_sign.js, tag=京东极速版免单签到, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

================Loon==============
[Script]
cron "20 0,22 * * *" script-path=jd_speed_md_sign.js,tag=京东极速版免单签到

===============Surge=================
京东极速版免单签到 = type=cron,cronexp="20 0,22 * * *",wake-system=1,timeout=3600,script-path=jd_speed_md_sign.js

============小火箭=========
京东极速版免单签到 = type=cron,script-path=jd_speed_md_sign.js, cronexpr="20 0,22 * * *", timeout=3600, enable=true
*/
const $ = new Env('京东极速版免单签到');
const notify = $.isNode() ? require('./sendNotify') : '';
//Node.js用户请在jdCookie.js处填写京东ck;
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';
let cookiesArr = [], cookie = '', message = '';
const linkId = 'PiuLvM8vamONsWzC0wqBGQ';
...
!(async () => {
  ...
  notify.sendNotify(`${$.name}`, message);//通知
})()
    .catch((e) => {
      $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
    })
...
async function jsRedPacket() {
  try {
    await signFreeHome();//获取签到列表
	$.tasksList = $.activityInfo.signFreeOrderInfoList;
	if ($.tasksList) {
      for (let i = 0; i < $.tasksList.length; i++) {
		$.orderId = $.tasksList[i].orderId;
	    $.productName = $.tasksList[i].productName;
		$.combination = $.tasksList[i].combination;
		if($.combination === 1){
           console.log(`任务：${$.productName},签到未开始`);
		   message += `任务：${$.productName},签到未开始\n`;
        } else if($.combination === 2){
		   console.log(`任务：${$.productName},今日已签到`);
		   message += `任务：${$.productName},今日已签到\n`;
		} else if($.combination === 3){
		   console.log(`任务：${$.productName},去执行签到`);
		   await sign();
		   await $.wait(1000);
		}
	  }
...
async function sign() {
  return new Promise(resolve => {
    const body = {"linkId":linkId,"orderId":$.orderId};
    const options = {
      url: `https://api.m.jd.com`,
      body: `functionId=signFreeSignIn&body=${escape(JSON.stringify(body))}&t=${+new Date()}&appid=activities_platform`,
      headers: {
        'Cookie': cookie,
        "Host": "api.m.jd.com",
        ...
        'Referer': `https://signfree.jd.com/?activityId=${linkId}`,
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = $.toObj(data);
            if (data.code === 0) {
                message += `极速版免单签到：签到成功\n`;
                console.log(`极速版免单签到：签到成功\n`);
            } else {
			    console.log(`极速版免单签到：${JSON.stringify(data)}\n`);
            }
          }
        }
      } catch (e) {
...
function signFreeHome() {
  return new Promise(resolve => {
    $.get(taskGetUrl("signFreeHome",{linkId}), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
